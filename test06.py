...
class MainWindow(QMainWindow):
    ...
    def start_process(self):
        print("START 按鈕被點擊")
        # 開啟新的終端並執行命令
        subprocess.Popen(['gnome-terminal', '--', 'bash', '-c', 'cd /home/sr/wheeltec_ros2/ && source install/setup.bash && ros2 launch wheeltec_nav2 wheeltec_nav2.launch.py'])

    @pyqtSlot()
    def stop_process(self):
        print("STOP 按鈕被點擊")
        # 您可以添加自動停止的指令或腳本
        subprocess.Popen(['gnome-terminal', '--', 'bash', '-c', 'pkill -f "ros2 launch wheeltec_nav2 wheeltec_nav2.launch.py"'])
