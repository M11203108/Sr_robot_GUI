...
class SimpleMapWindow(QMainWindow):
    def __init__(self, yaml_files, pgm_files):
        ...
        self.current_map_index = -1  # 初始值設為-1，表示沒有選擇地圖
    ...
    def initUI(self):
        ...
        map_selector.addItem('选择地图')  # 添加默认的选项
        ...
        self.label.setFixedSize(1000, 1000)  # 设置固定的大小
        self.label.setStyleSheet("background-color: lightgray;")  # 使用灰色背景来表示初始空白状态

        # 创建显示坐标的标签
        self.coord_label = QLabel(self)
        self.coord_label.setFixedSize(200, 50)
        self.coord_label.setStyleSheet("background-color: white; border: 1px solid black; font-size: 18px;")
    ...
    def switch_map(self, index):
        """切换地图"""
        if index == 0:  # 如果选择了默认的 '选择地图' 选项
            self.label.clear_map()
            return

        self.current_map_index = index - 1  # 调整索引以匹配文件列表
    ...
    def update_coords(self, coords):
        """更新左下角的坐标显示"""
        self.coord_label.setText(f"坐标: ({coords[0]:.2f}, {coords[1]:.2f})")
...
class MapLabel(QLabel):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if self.map_metadata:
            world_coords = self.pixel_to_world(event.pos())
            self.parent.update_coords(world_coords)
    ...
    def drawOrigin(self, painter):
        """绘制地图原点"""
        if not self.map_metadata or not self.pixmap():
            return
    ...
    def pixel_to_world(self, pos):
        ...
        map_origin_x, map_origin_y = self.map_metadata['origin'][0], self.map_metadata['origin'][1]
        resolution = self.map_metadata['resolution']
        x = map_origin_x + pos.x() * resolution / self.scale_factor
        y = map_origin_y + (self.pixmap().height() - pos.y() / self.scale_factor) * resolution
        return x, y
