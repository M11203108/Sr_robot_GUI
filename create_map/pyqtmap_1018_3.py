...
class MapBuilderWindow(QMainWindow):
    ...
    def zoom_in(self):
        self.current_scale *= 2  # 每次放大倍率翻倍
        self.update_map_transform()

    def zoom_out(self):
        self.current_scale /= 2  # 每次縮小倍率減半
        self.update_map_transform()
    ...
    def update_scale_label(self):
        if self.resolution is not None:
            ...
            # 根據 10m, 5m, 1m, 50cm, 10cm 的倍數來顯示
            if scale_length_m >= 10:
                rounded_length = round(scale_length_m / 10) * 10
                units = 'm'
            elif scale_length_m >= 1:
                rounded_length = round(scale_length_m / 5) * 5
                units = 'm'
            elif scale_length_m >= 0.1:
                rounded_length = round(scale_length_m, 1)
                units = 'm'
            elif scale_length_m >= 0.05:
                rounded_length = 50
                units = 'cm'
            else:
                rounded_length = 10
            ...
            self.scale_label.setText(f"比例尺: {rounded_length:.0f} {units} (50 像素)")
        else:
