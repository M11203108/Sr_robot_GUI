...
class MapBuilderWindow(QMainWindow):
    ...
    def draw_scale(self, painter):
        ...
        # 計算比例尺長度（動態隨縮放比例調整）
        scale_length_cm = 50 / self.current_scale  # 這裡假設 50 像素對應 50 公分（未縮放時）
        if scale_length_cm < 100:
            units = 'cm'
            display_length = int(scale_length_cm)
        else:
            units = 'm'
            display_length = scale_length_cm / 100

        # 固定比例尺在右下角
        painter.drawLine(self.map_label.width() - 110, self.map_label.height() - 30,
                         self.map_label.width() - 60, self.map_label.height() - 30)
        painter.drawText(self.map_label.width() - 110, self.map_label.height() - 35, f"{display_length:.1f} {units}")
