...
class MapBuilderWindow(QMainWindow):
    ...
    def update_map_transform(self):
        if self.current_map_path:
            ...
            scaled_pixmap = rotated_pixmap.scaled(
                int(rotated_pixmap.width() * self.current_scale),
                int(rotated_pixmap.height() * self.current_scale),
                Qt.KeepAspectRatio,
    ...
    def update_scale_label(self):
        if self.resolution is not None:
            ...
            # 使比例尺以 0.5 米為增量顯示
            rounded_length = round(scale_length_m * 2) / 2  # 四捨五入到最接近的 0.5 倍數
            
            # 確保顯示為米並四捨五入到一位小數
            self.scale_label.setText(f"比例尺: {rounded_length:.1f} m (50 像素)")
        else:
