...
class MapWindow(QMainWindow):
    def __init__(self, yaml_file, pgm_files, waypoints):
        ...
        # 創建地圖顯示區域
        self.label = MapLabel(self.origin, self.resolution, self.load_image_height(pgm_files[0]), pgm_files[0], waypoints)
...
class MapLabel(QLabel):
    def __init__(self, origin, resolution, image_height, pgm_file, waypoints):
        super().__init__()
        self.origin = origin
    ...
    def paintEvent(self, event):
        ...
        for waypoint in self.waypoints:
            # 將世界座標轉換為像素座標
            px_x = (waypoint['x'] - self.origin[0]) / self.resolution
            px_y = self.image_height - ((waypoint['y'] - self.origin[1]) / self.resolution)
