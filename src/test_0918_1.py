...
class MapWindow(QMainWindow):
    ...
    def save_points(self):
        ...
        # 計算總停留時間（秒）
        stay_duration_total = (self.stay_duration_hours.value() * 3600 +
                            self.stay_duration_minutes.value() * 60 +
                            self.stay_duration_seconds.value())

        # 遍歷每個點，並將數值限制到小數點後 15 位，並新增停留時間欄位
        rounded_points = []
        for point in self.recorded_points:
            rounded_point = {
                ...
                "stay_duration": stay_duration_total  # 儲存總停留時間
            }
            rounded_points.append(rounded_point)

        # 儲存格式化後的點位
        with open('saved_points.json', 'w') as file:
            json.dump({'points': rounded_points}, file, indent=4, separators=(',', ': '))
    ...
    def update_map(self, index):
        """更新顯示的地圖影像和元資料，並呼叫 map_server 服務來加載地圖"""
        if index == 0:
            # 空白選項，隱藏地圖
            self.label.clear()
            return
