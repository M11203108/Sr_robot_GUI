...
class MapWindow(QMainWindow):
    ...
    #     """更新顯示的地圖影像和元資料"""
    #     if index == 0:
    #         # 如果選擇了 "Select a map"，清除地圖
    #         self.label.clear_map()
    #         return
    ...
    #         if not os.path.exists(selected_pgm_file) or not os.path.exists(selected_yaml_file):
    #             raise FileNotFoundError(f"地圖檔案不存在: {selected_pgm_file} 或 {selected_yaml_file}")
    ...
    #             selected_pgm_file,
    #             self.load_image_height(selected_pgm_file),
    #             self.origin,
    #             self.resolution
    #         )
    #         print(f"成功更新地圖: {selected_pgm_file}")
    ...
    #         ]
    #         subprocess.run(command, check=True)
    #         print(f"成功加載地圖至 ROS2: {selected_yaml_file}")

    #     except Exception as e:
    #         print(f"更新地圖時發生錯誤: {e}")
    #         QMessageBox.critical(self, "錯誤", f"無法更新地圖: {e}")
...
class MapLabel(QLabel):
    ...
    def update_image(self, pgm_file, image_height, origin, resolution, scale_factor=1):
        ...
        try:
            ...
            if qimage.isNull():
                print(f"載入地圖影像失敗: {pgm_file}")
                return

            # 計算縮放後的大小
            w = int(qimage.size().width() * scale_factor)
            h = int(qimage.size().height() * scale_factor)

            # 使用 QPixmap 來顯示地圖影像
            pixmap = QPixmap.fromImage(qimage).scaled(w, h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.setPixmap(pixmap)

            print(f"成功更新地圖: {pgm_file}")
        except Exception as e:
            print(f"更新地圖時發生錯誤: {e}")
    ...
    def paintEvent(self, event):
        ...
        for i, point in enumerate(self.points):
            if i % 2 == 0:  # 只在每對點的起點繪製紅色圓點
                # 使用QPointF並縮放
                scaled_point = QPointF(point.x() * self.scale_factor, point.y() * self.scale_factor)
                painter.drawEllipse(scaled_point, 5, 5)  # 5x5 像素的紅色圓點

        # 繪製所有方向
        for j, (start_point, end_point) in enumerate(self.directions):
            # 使用QPointF並縮放
            start_point_scaled = QPointF(start_point.x() * self.scale_factor, start_point.y() * self.scale_factor)
            end_point_scaled = QPointF(end_point.x() * self.scale_factor, end_point.y() * self.scale_factor)

            painter.setPen(Qt.NoPen)
            painter.setBrush(QColor("red"))
            painter.drawEllipse(start_point_scaled, 5, 5)  # 5x5 像素的紅色圓點

            # 繪製箭頭
            painter.setPen(QColor("blue"))
            painter.setBrush(Qt.NoBrush)

            arrow_length = 50 * self.scale_factor  # 調整箭頭長度，根據縮放比例
            angle = math.atan2(end_point_scaled.y() - start_point_scaled.y(), end_point_scaled.x() - start_point_scaled.x())
            adjusted_end = start_point_scaled + QPointF(
                arrow_length * math.cos(angle),
                arrow_length * math.sin(angle)
            )

            painter.drawLine(start_point_scaled, adjusted_end)
            self.draw_arrow_head(painter, start_point_scaled, adjusted_end)
