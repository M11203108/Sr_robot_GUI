...
import json
import math
from PyQt5.QtWidgets import QApplication, QLabel, QMainWindow, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QComboBox, QInputDialog, QFileDialog
...
class MapWindow(QMainWindow):
    def __init__(self, yaml_files, pgm_files):
        ...
        self.save_file_button.setFixedSize(120, 60)
        ...
        file_buttons_layout.addWidget(self.cancel_file_button)
        ...
        # 將 file_buttons_layout 添加到按鈕總佈局
        button_layout.addLayout(file_buttons_layout)
    ...
    def save_file_with_name(self):
        ...
        if ok and file_name:
            save_path = os.path.expanduser(f"~/gui_ws/{file_name}.json")
    ...
    def cancel_file(self):
        ...
        self.label.points.clear()
        self.label.directions.clear()
        self.label.update()  # 更新地圖顯示
        print("已取消載入的檔案，所有點位已清空")
...
class MapLabel(QLabel):
    ...
    #     """更新地圖上的點位和方向"""
    #     self.points = []  # 清空原本的點位
    #     self.directions = []  # 清空原本的方向
        
    #     for point in points:
    #         # 將世界座標轉換為像素座標
    #         world_pos = QPointF(point['x'], point['y'])
    ...
    def update_image(self, pgm_file, image_height, origin, resolution):
        ...
        self.scale_factor = scaled_pixmap.width() / pixmap.width()  # 計算新的縮放比例
        self.setPixmap(scaled_pixmap)
        self.update()  # 觸發重繪
