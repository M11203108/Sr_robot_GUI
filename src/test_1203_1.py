...
class MapWindow(QMainWindow):
    ...
    def start_process(self):
        ...
        if self.terminal_process is None or self.terminal_process.poll() is not None:
            # 啟動前更新 JSON 檔案
            default_json_path = "/home/sr/wheeltec_ros2/src/wheeltec_robot_nav2/saved_points.json"
    ...
    def save_points(self):
        ...
        with open('/home/sr/wheeltec_ros2/src/wheeltec_robot_nav2/saved_points.json', 'w') as file:
            json.dump({'points': rounded_points}, file, indent=4)
...
class MapLabel(QLabel):
    ...
    def update_image(self, pgm_file, image_height, origin, resolution,scale_factor=1):
        ...
        # 計算縮放後的寬度和高度
        scaled_width = int(qimage.width() * scale_factor)
        scaled_height = int(qimage.height() * scale_factor)

        # 創建一個 QPixmap 作為繪圖目標
        pixmap = QPixmap(scaled_width, scaled_height)
        pixmap.fill(Qt.transparent)  # 填充為透明背景

        # 更新 QLabel 的 pixmap
        self.setPixmap(pixmap)
        # 使用 QPainter 繪製圖片
        painter = QPainter(pixmap)
        painter.drawImage(QRect(0, 0, scaled_width, scaled_height), qimage)
        painter.end()
