...
class MapWindow(QMainWindow):
    def __init__(self, yaml_files, pgm_files):
        ...
        # 將按鈕佈局、放大縮小按鈕佈局和地圖佈局新增至主佈局
        content_layout = QHBoxLayout()
        content_layout.addLayout(button_layout)
        content_layout.addLayout(zoom_layout)  # 放大縮小按鈕位於按鈕佈局與地圖顯示區域之間
    ...
    def zoom_in(self):
        self.label.scale_factor *= 1.2  # 放大 20%
        self.label.update_image(self.label.current_pgm_file)  # 更新地圖影像

    def zoom_out(self):
        self.label.scale_factor /= 1.2  # 縮小 20%
        self.label.update_image(self.label.current_pgm_file)  # 更新地圖影像
    ...
    def start_process(self):
        ...
        # 啟動進程並保存進程對象
            ...
            self.terminal_process = subprocess.Popen([
                "xterm", "-e", "bash -c 'cd ~/wheeltec_ros2 && source install/setup.bash && /bin/python3.10 /home/sr/wheeltec_ros2/src/wheeltec_robot_nav2/launch/waypoint_testgui_time.py; exec bash'"
            ])
    ...
    def load_saved_points(self):
        ...
        try:
            # 使用絕對路徑
            file_path = os.path.expanduser('~/gui_ws/saved_points.json')
    ...
    def update_map(self, index):
        ...
        # 更新顯示的地圖
        self.label.update_image(selected_pgm_file)  # 只傳遞 pgm_file
...
class MapLabel(QLabel):
    def __init__(self, parent):
        ...
        self.current_pgm_file = None  # 用於保存當前地圖的檔案路徑
        self.points = []  # 儲存所有點擊的位置
        self.directions = []  # 儲存所有的方向箭頭

    def update_image(self, pgm_file=None):
        """更新顯示的地圖影像，並自動縮放以適應視窗"""
        if pgm_file:
            self.current_pgm_file = pgm_file  # 保存當前載入的地圖檔案路徑
        if not self.current_pgm_file:
            return  # 如果沒有載入地圖則退出

        image = QPixmap(self.current_pgm_file)
