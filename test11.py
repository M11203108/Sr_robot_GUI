...
class MapWidget(QLabel):
    def __init__(self):
        ...
        self.map_origin = (-10, -10)  # 地圖原點在實際世界的座標 (以米為單位)
        self.map_resolution = 0.05  # 每個像素的解析度 (米/像素)
    ...
    def mousePressEvent(self, event):
        ...
        # 計算在地圖上的實際座標
        map_pos_x = (x_click - self.width() / 2) * self.map_resolution + self.map_origin[0]
        map_pos_y = (y_click - self.height() / 2) * self.map_resolution + self.map_origin[1]

        # 將目標座標發送到 ROS 2 系統作為導航目標
        print(f"目標座標: ({map_pos_x}, {map_pos_y})")
        self.send_goal_to_robot(map_pos_x, map_pos_y)

    def send_goal_to_robot(self, x, y):
        # 這裡可以整合 ROS 2 的 Action Client 來發送目標點
        # 使用 nav2_simple_commander 或者自己實作一個 Action 客戶端
        pass
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.rviz2_button = QPushButton("RViz2")
        self.point_button = QPushButton("指定點位")

        # 設定按鈕大小和樣式
        self.start_button.setFixedSize(150, 150)
        self.stop_button.setFixedSize(150, 150)
        self.home_button.setFixedSize(150, 150)
        self.rviz2_button.setFixedSize(150, 150)
        self.point_button.setFixedSize(150, 150)
        ...
        self.rviz2_button.setStyleSheet("background-color: orange; color: white; font-size: 30px;")
        self.point_button.setStyleSheet("background-color: purple; color: white; font-size: 30px;")

        # 綁定按鈕事件
        self.start_button.clicked.connect(self.start_process)
        self.stop_button.clicked.connect(self.stop_process)
        self.home_button.clicked.connect(self.home_process)
        self.rviz2_button.clicked.connect(self.open_rviz2)
        self.point_button.clicked.connect(self.set_point)
        ...
        button_layout.addWidget(self.rviz2_button)
    ...
    def open_rviz2(self):
        ...
        # 在系統環境中開啟新的終端機執行指令
        subprocess.Popen([
            # "xterm", "-e", "bash -c 'cd ~/wheeltec_ros2 && source install/setup.bash && ros2 run rviz2 rviz2; exec bash'"
            "xterm", "-e", "bash -c 'cd ~/wheeltec_ros2 && source install/setup.bash && ros2 launch wheeltec_rviz2 wheeltec_rviz.launch.py; exec bash'"
        ])

    @pyqtSlot()
    def set_point(self):
        print("指定點位 按鈕被點擊")
        # 這裡可以添加執行指定點位時需要執行的命令
