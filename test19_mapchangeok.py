...
class MapWindow(QMainWindow):
    def __init__(self, yaml_file, pgm_files, waypoints):
        ...
        # 創建地圖顯示區域
        self.label = MapLabel(self.origin, self.resolution, self.load_image_height(pgm_files[0]), pgm_files[0], waypoints, scale_factor=2)#圖片比例設定
...
class MapLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        for waypoint in self.waypoints:
            ...
            px_y = (self.image_height - (waypoint['y'] - self.origin[1]) / self.resolution) * self.scale_factor
            ...
            angle = 2 * math.atan2(waypoint['qz'], waypoint['qw'])  # 计算旋转角度
