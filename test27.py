...
class MapLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        print("paintEvent called")  # 確認每次都會進行繪製
        ...
        for start_point, end_point in self.directions:
            # 将世界坐标转换为像素坐标进行绘制
            sp = self.parent().world_to_pixel(start_point[0], start_point[1])
            ep = self.parent().world_to_pixel(end_point[0], end_point[1])
            print(f"Drawing arrow from {sp} to {ep}")  # 確認箭頭座標
            painter.drawLine(sp, ep)
            self.draw_arrow_head(painter, sp, ep)
